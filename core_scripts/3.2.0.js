// AutoSuite CORE v3.3.2 (KEEP UI + Fix date table + Fix Select2 driver + Overtime NET + RP 9h rule)
(function(){
'use strict';
...
function requireToken(token){ if(!state.running || token!==state.runToken) throw ABORT; }
...
function loadProfiles(){
  try{ return JSON.parse(GM_GetValueSafe(KEYS.profiles,'{}')||'{}'); }catch{return {};}
}
...
function initSigPad(){
  ...
  $('#mt_sig_clear')?.addEventListener('click', ()=>{
    dashSig.strokes=[]; dashSig.cur=[]; dashSig.hasInk=false;
...
function getSelect2Container(){
  // your provided HTML:
  // <span class="select2-selection__rendered" id="select2-staff_id-container"...>Name</span>
  return $('#select2-staff_id-container') || $('span.select2-selection__rendered[id*="staff_id"]');
}
function getDriverSelect(){
  // underlying select usually exists even with Select2
...
async function selectDriverByNameSelect2UI(name, token){
  // Fallback: open select2 dropdown, type, pick first match
  ...
  let best = options.find(o=>normName(o.textContent||'')===target) || options[0];
...
async function selectDriverByName(name, token){
  ...
  if(sel && sel.options && sel.options.length){
    for(let i=0;i<sel.options.length;i++){
      ...
      if(normName(txt)===target){
        ...
        // support jQuery/select2 if present
        try{
          if(window.jQuery && jQuery(sel).data('select2')){
            jQuery(sel).val(sel.options[i].value).trigger('change');
          }else{
  ...
  // fallback: use select2 UI typing
...
function getDashStartKmInt(){ return parseKmInt(formatDashKmDot00($('#mt_km')?.value||'')); }
...
async function waitForReady(driverName, token){
  ...
  while(Date.now()-t0<5000){
    requireToken(token);
    if(isDashboardReady()){ setReady(true); return true; }
...
function findTimeTable(){
  // Prefer a DataTables-like table: rows with td.sorting_1 containing dates
  ...
  // fallback: scan tables by count of date matches
  const tables = $$('table');
...
function findEditButton(tr){
  // your original environment often uses pencil / bearbeiten.
  ...
  for(const el of candidates){
    ...
    const ok =
      ...
      dt.includes('modal') ||
      title.includes('bearbeit') || title.includes('edit') ||
      t.includes('bearbeit') || t.includes('details') ||
      href.includes('edit') || href.includes('bearbeit') || href.includes('detail');
    if(ok) return el;
  }
  // fallback: try any button in row
...
function initMonthPlansFromTable(table){
  ...
  const ot=getOtRangeFromDash();
  const overtimeTarget = randInt(Math.round(ot.min*60), Math.round(ot.max*60)); // minutes
  ...
  for(let pass=0; pass<6 && Math.abs(delta)>8; pass++){
    for(const dateStr of shuffled){
      ...
      const plan=plans[dateStr]; if(!plan) continue;
...
function genPlanUPS(dateStr){
  ...
  if (wantMin > wantMax) { wantMin = Math.max(END_MIN, start + DUR_MIN); wantMax = Math.max(wantMin, Math.min(END_MAX, start + DUR_MAX)); }
...
function genPlanDPD(dateStr){
  ...
  if (wd === 1) { sMin=timeStrToMins('05:55'); sMax=timeStrToMins('06:05'); eMin=timeStrToMins('14:25'); eMax=timeStrToMins('14:35'); }
  else if (wd === 2) { sMin=timeStrToMins('05:25'); sMax=timeStrToMins('05:40'); eMin=timeStrToMins('14:55'); eMax=timeStrToMins('15:10'); }
  else if (wd === 3) { sMin=timeStrToMins('05:25'); sMax=timeStrToMins('05:40'); eMin=timeStrToMins('14:55'); eMax=timeStrToMins('15:10'); }
  else { sMin=timeStrToMins('05:25'); sMax=timeStrToMins('05:40'); eMin=timeStrToMins('13:55'); eMax=timeStrToMins('14:10'); }
...
/* ---------------- Completion helpers (same as before) ---------------- */
...
const CLEAN_CFG={ pauseRowSel:'#pause_dataList_body > tr', deleteBtnSel:'a[title="Löschen"]', swalConfirmSel:'.swal2-confirm' };
...
async function cleanAllPagesForCurrentDriver(token){
  // keeps your original behavior; add pagination loop later if needed
...
async function runMultiDrivers(token){
  ...
  for(const name of list){
    ...
    if(!ok){ log(`❌ Fahrer nicht gefunden im Dropdown: ${name}`); continue; }
